...
def process_ugg_file(filename):
    ...
    def get_data_from_frame(frame):
        if frame[5] != 0:
            ...
            return frame[
                   44:]  # at byte 44 the data starts, hard to say what is the first 44bytes, because it doesn't really conform the Ogg stadard
    ...
    frame_start = s.find(b'OggS',
                         0)  # The UGG format is similar to Ogg format, but not compatible. But in The UGG files, OggS marks new page of data
    while frame_start != -1:
